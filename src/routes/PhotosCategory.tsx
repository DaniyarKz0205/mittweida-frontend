...
const fakeRoutes: RouteType[] = [
    {
        id: 7,
        title: 'Ut auctor',
        coords: [51.00799225684178, 12.977633720686717],
        distance: '2.5 km',
        duration: '2h',
        stops: 3,
        rating: 4.7,
...
export default function PhotosCategory() {
    ...
    const UtAuctor = fakeRoutes.find(r => r.id === 7);
    ...
    return (
        <div style={{
            ...
            <div className="category-header">
                ...
                <h2 className="header-title">Hidden Photo Spots Routes</h2>
            </div>
                ...
                {UtAuctor && (
                    <div
                        ...
                        style={{
                            ...
                            overflow: 'hidden',
                            paddingBottom: selected?.id === UtAuctor.id ? '16px' : '0px'
                        }}
                    >
                        <img src={UtAuctor.image} alt={UtAuctor.title}
                             style={{width: '100%', height: 140, objectFit: 'cover'}}/>
                        <div style={{padding: '12px 16px'}}>
                            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                                <h3 style={{margin: 0}}>{UtAuctor.title}</h3>
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        navigate('/photos-info', {state: UtAuctor});
                                    }}
                                    style={{
                                        background: 'none',
                                        border: 'none',
                            ...
                            <p>{UtAuctor.distance} • {UtAuctor.duration} • {UtAuctor.stops} stops</p>
                            <p><span style={{color: '#f5b100'}}>{UtAuctor.rating} ★★★★★</span> ({UtAuctor.reviews})
                            </p>
                            {selected?.id === UtAuctor.id && (
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        navigate('/photos-1navigate');
                                    }}
                                    style={{
                                        marginTop: '10px',
