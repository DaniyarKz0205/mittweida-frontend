...
const fakeRoutes: RouteType[] = [
    {
        ...
        coords: [50.98691174536631, 12.974221950874893],
        distance: '3 km',
        duration: '2h',
        stops: 3,
        rating: 4.2,
...
export default function FoodCategory() {
    ...
    const Aenean = fakeRoutes.find(r => r.id === 5);
    ...
    return (
        <div style={{
            ...
            <div className="category-header">
                ...
                <h2 className="header-title">Food & Drink Routes</h2>
            </div>
                ...
                {Aenean && (
                    <div
                        ...
                        style={{
                            ...
                            overflow: 'hidden',
                            paddingBottom: selected?.id === Aenean.id ? '16px' : '0px'
                        }}
                    >
                        <img src={Aenean.image} alt={Aenean.title}
                             style={{width: '100%', height: 140, objectFit: 'cover'}}/>
                        <div style={{padding: '12px 16px'}}>
                            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                                <h3 style={{margin: 0}}>{Aenean.title}</h3>
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        navigate('/food-info', {state: Aenean});
                                    }}
                                    style={{
                                        background: 'none',
                                        border: 'none',
                            ...
                            <p>{Aenean.distance} • {Aenean.duration} • {Aenean.stops} stops</p>
                            <p><span style={{color: '#f5b100'}}>{Aenean.rating} ★★★★★</span> ({Aenean.reviews})
                            </p>
                            {selected?.id === Aenean.id && (
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        navigate('/food-1navigate');
                                    }}
                                    style={{
                                        marginTop: '10px',
